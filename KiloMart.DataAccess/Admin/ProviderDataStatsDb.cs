...
namespace KiloMart.DataAccess.Admin
{
    public static partial class Stats
    {
        ...
        WITH ProviderData AS (
            ...
            WHERE pp.IsActive = 1 AND (@IsActive IS NULL OR m.IsActive = @IsActive)
        ...
        FETCH NEXT @PageSize ROWS ONLY;";
            ...
            return await connection.QueryAsync<ProviderDataDto>(dataQuery, new { Page = page, PageSize = pageSize, SearchTerm = searchTerm, IsActive = isActive });
        }
        public static async Task<int> GetActiveFilteredProvidersProfilesCountAsync(IDbConnection connection, bool? isActive, string? searchTerm = null)
        {
            const string countQuery = @"
            SELECT COUNT(*) 
                FROM ProviderProfileHistory pp
