...
public partial class Query
{
    ...
    // Pagination function to get total count of products
    // public static async Task<int> GetTotalProductsCount(
    //     IDbConnection connection, 
    //     int provider, 
    //     byte language, 
    //     int? categoryId = null)
    ...
    //     var countQuery = @"
    //         SELECT COUNT(*)
    //         FROM GetProductDetailsFN(@language) pd
    //         LEFT JOIN dbo.[ProductOffer] po ON 
    //             pd.ProductId = po.Product AND po.Provider = @provider AND po.IsActive = 1
    //         WHERE 
    ...
    // Pagination function to get products with offset and limit
