...
public class LoginService
{
    public static async Task<LoginResult> Login(string email, string password, IDbFactory dbFactory, IConfiguration configuration)
    {
        ...
        var sessionCode = Guid.NewGuid().ToString();
        var (token, expires) = JwtTokenHandler.GenerateJwtToken(user, sessionCode, configuration);
        ...
        loginResult = await _handleProfile(connection, user, loginResult);
        
        await Db.InsertSessionAsync(
            connection,
            token,
            loginResult.UserId,
            expires,
    ...
    private static async Task<LoginResult> _handleProfile(IDbConnection connection, MembershipUser user, LoginResult loginResult)
    {
        IProfileHandler? profileHandler = user.Role switch
        {
        ...
        {
            loginResult = await profileHandler.HandleProfileAsync(loginResult, connection);
        }
    ...
    private static string CheckRole(short role)
    {
        return role switch
        {
            (short)Roles.Customer => "Customer",
            (short)Roles.Admin => "Admin",
            (short)Roles.Provider => "Provider",
            (short)Roles.Delivery => "Delivery",
            _ => throw new Exception("Not Supported Role")
        };
    }
